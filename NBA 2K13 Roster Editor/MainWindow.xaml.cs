        ...
        private void PopulatePlayersTab()
        {
            ...
            using (brOpen = new RosterReader(new MemoryStream(File.ReadAllBytes(currentFile))))
            {
                for (int i = 0; i <= Convert.ToInt32(GetOption("LastPlayerID")); i++)
                {
                    ...
                    pe.ID = i;
                    PlayerReader.ReadPlayerInfo(ref pe, brOpen);
                    pe.Name = FindPlayerName(pe);
                    pe.SSList = PlayerReader.ReadSignatureSkills(i, brOpen);
                    pe.Ratings = PlayerReader.ReadRatings(i, brOpen);
                    pe.Tendencies = PlayerReader.ReadTendencies(i, brOpen);
                    pe.HotSpots = PlayerReader.ReadHotSpots(i, brOpen);
                    pe.HotZones = PlayerReader.ReadHotZones(i, brOpen);
                    pe.Accessories = PlayerReader.ReadGear(i, brOpen);
